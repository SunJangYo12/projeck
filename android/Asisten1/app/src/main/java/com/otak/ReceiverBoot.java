...
public class ReceiverBoot extends BroadcastReceiver
{
	...
	public void onReceive(Context context, Intent intent)
	{
		...
		if (intent.getAction().equals(Intent.ACTION_BOOT_COMPLETED))
		{
			...
			sertts.cepat = 1.0f;
			sertts.str = "selamat datang,tuan";
		...
		if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED))
		{
			...
			if (BatteryTemp >= 36.5){
				toastText(context, dataTemp, Color.GREEN, Gravity.TOP | Gravity.RIGHT);
			}
			if (voltase <= 34.5){
		...
		if (intent.getAction().equals(Intent.ACTION_SCREEN_OFF))
		{
			if (settings.getBoolean("Mic Control", false)){
				context.startService(new Intent(context, ServiceMicHelper.class));
				sh.inte = true;
				layar = 1;
			}
		}
		...
		{
			if (settings.getBoolean("Mic Control", false)){
				context.stopService(new Intent(context, ServiceMicHelper.class));
				layar = 0;
			}
		}
